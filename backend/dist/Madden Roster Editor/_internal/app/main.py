...
from app.fallback_labels import FALLBACK_PLAY_LABELS, FALLBACK_TEAM_LABELS
...
app = FastAPI(title="Madden Roster Editor API", version="0.1.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:3000",
...
def get_field_labels_for_table(table_name: str) -> Dict[str, str]:
    ...
    if table_name == "PLAY":
        ...
        return labels or FALLBACK_PLAY_LABELS
    if table_name == "TEAM":
        labels = get_amp_constant_labels(str(AMP_TEAM_RECORD))
        return labels or FALLBACK_TEAM_LABELS
    return {}
...
def table_columns(session_id: str, table_path: str, records: List[Dict[str, Any]]) -> List[str]:
    enriched = [enrich_record(session_id, table_path, r) for r in records[:100]]
    cols = collect_columns(enriched or records)
    # Put team and position next to the IDs when present.
    def move_after(cols: List[str], target: str, after: str) -> List[str]:
        if target in cols:
            cols = [c for c in cols if c != target]
            idx = cols.index(after) + 1 if after in cols else 0
            cols.insert(idx, target)
        return cols
    cols = move_after(cols, "TeamName", "TGID")
    cols = move_after(cols, "Position", "PPOS")
    return cols
...
@app.get("/api/health")
def health():
    return {"ok": True, "app": "Madden Roster Editor API"}
...
def get_table(
    ...
    sample_records = [row for _, row in enriched_rows[:100]]
    columns = table_columns(session_id, table_path, page or sample_records or records[:100])
...
def paste_grid(session_id: str, req: BulkPasteRequest):
    ...
    records = obj.get("records", [])
    cols = table_columns(session_id, req.table_path, records)
...
def serve_frontend_root():
    if not can_serve_frontend():
        return HTMLResponse(
            "<h1>Madden Roster Editor API</h1><p>Frontend build not found. Run <code>npm.cmd --prefix frontend run build</code>.</p>",
            status_code=200,
        )
